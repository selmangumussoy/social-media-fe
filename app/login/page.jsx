    ...
    try {
      ...
      if (token) {
        if (user) {
          ...
          // Fallback: Eğer user null ise, basit bir user objesi oluştur veya /me çağır
          // Burada geçici olarak username'i user objesi gibi set ediyoruz ki null hatası almayalım
          // İdeal olan: const userDetails = await getUserDetails(); dispatch(setUser(userDetails));
          console.warn("Login yanıtında user objesi yok, manuel oluşturuluyor.");
          dispatch(setUser({ username: username, id: "temp-id-" + Date.now() }));
        }
