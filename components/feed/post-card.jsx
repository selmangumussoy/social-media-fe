      ...
      <CardContent className="space-y-4 pb-3">
        {post.type === "quote" && (
          <div className="relative rounded-xl bg-gradient-to-br from-purple-50/50 to-blue-50/50 p-6">
            ...
            <p className="relative z-10 font-serif text-lg leading-relaxed text-foreground/90">{post.content}</p>
            {post.pageNumber && (
        ...
        {post.type === "quote" && post.bookInfo && (
          <div className="flex items-center gap-3 rounded-xl border border-border/50 bg-white p-3">
            ...
            <div className="flex-1 min-w-0">
              <h4 className="font-semibold text-sm text-foreground truncate">{post.bookInfo.title}</h4>
              <p className="text-xs text-muted-foreground truncate">{post.bookInfo.author}</p>
      ...
      <CardFooter className="flex items-center justify-between border-t border-border/30 pt-3 pb-3">
        <div className="flex items-center gap-1">
          ...
          >
            ...
            <span className="text-sm">{post.shares || 234}</span>
          </Button>
        </div>
        ...
        >
          ...
          {isSaved && <span className="ml-1.5 text-sm">{post.saves || 456}</span>}
        </Button>
      </CardFooter>
    </Card>
  )
