...
import { Heart, MessageCircle, Bookmark, Share2, MoreHorizontal, Quote, Edit, Trash2, Check, X, BookOpen, ArrowRight } from "lucide-react"
...
export function PostCard({ post }) {
  ...
  const handleDelete = async () => {
    ...
    // 👇 YENİ MANTIK: İÇERİK UZUNLUĞU KONTROLÜ
    const STRIP_HTML_REGEX = /<[^>]*>?/gm;
    const MAX_LENGTH = 180;

    // 1. HTML'den temizlenmiş saf metni al
    const rawContent = post.content ? post.content.replace(STRIP_HTML_REGEX, "") : "";

    // 2. Metin uzun mu diye kontrol et
    const isLongContent = rawContent.length > MAX_LENGTH;

    // 3. Gösterilecek metni belirle (Uzunsa kes, kısaysa olduğu gibi bırak)
  ...
  const handleSave = async (e) => {
    ...
    const handleInlineUpdate = async () => {
        ...
        try {
            setIsUpdating(true);
            await updatePost(post.id, { content: editContent, title: post.title, type: post.type });
            toast.success("Güncellendi ✅"); setIsEditing(false); window.location.reload();
        } catch (error) { console.error(error); toast.error("Güncellenemedi"); }
        ...
        <CardContent className="space-y-4 px-5 py-2">
          {isEditing ? (
            ...
            <CardContent className="px-5 py-2 space-y-4">
                ...
                {/* ========================================================= */}
                {/* 1. BLOG POST (AKILLI DEVAMINI OKU ÖZELLİĞİ)               */}
                {/* ========================================================= */}
                {post.type === "BLOG_POST" && (
                    <div
                    ...
                    >
                        ...
                        {/* 👇 SADECE METİN UZUNSA GÖSTERİLECEK KISIM */}
                        {isLongContent && (
                    ...
                    <>
                        {isEditing ? (
                            <div className="space-y-3 animate-in fade-in zoom-in-95 duration-200">
                                ...
                                />
                                <div className="flex justify-end gap-2">
                                    <Button size="sm" variant="outline" onClick={() => setIsEditing(false)} disabled={isUpdating}><X className="h-4 w-4 mr-1" /> İptal</Button>
                                    <Button size="sm" onClick={handleInlineUpdate} disabled={isUpdating} className="bg-teal-700 hover:bg-teal-800 text-white">{isUpdating ? "..." : <><Check className="h-4 w-4 mr-1" /> Kaydet</>}</Button>
                                </div>
                            </div>
                        ) : (
